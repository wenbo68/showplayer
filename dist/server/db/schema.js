...
import { index, integer, jsonb, pgEnum, pgTable, primaryKey, real, text, timestamp, uniqueIndex, varchar, boolean, } from 'drizzle-orm/pg-core';
...
export const tmdbMediaToTmdbOrigin = pgTable('tmdb_media_to_tmdb_origin', {
...
export const tmdbMediaToTmdbOriginRelations = relations(tmdbMediaToTmdbOrigin, ({ one }) => ({
    media: one(tmdbMedia, {
...
export const tmdbMediaToTmdbGenre = pgTable('tmdb_media_to_tmdb_genre', {
    mediaId: varchar('media_id', { length: 255 })
        .notNull()
        .references(() => tmdbMedia.id, { onDelete: 'cascade' }),
...
export const mediaToGenresRelations = relations(tmdbMediaToTmdbGenre, ({ one }) => ({
    media: one(tmdbMedia, {
...
export const tmdbRecommendation = pgTable('tmdb_recommendation', {
...
export const tmdbRecommendationRelations = relations(tmdbRecommendation, ({ one }) => ({
...
export const tmdbSeason = pgTable('tmdb_season', {
    id: varchar({ length: 255 })
        .notNull()
        .primaryKey()
        .$defaultFn(() => crypto.randomUUID()),
...
export const tmdbEpisode = pgTable('tmdb_episode', {
    id: varchar({ length: 255 })
        .notNull()
        .primaryKey()
        .$defaultFn(() => crypto.randomUUID()),
...
export const tmdbSource = pgTable('tmdb_source', {
    id: varchar({ length: 255 })
        ...
        .primaryKey()
        .$defaultFn(() => crypto.randomUUID()),
    mediaId: varchar('media_id', { length: 255 }).references(() => tmdbMedia.id, {
        onDelete: 'cascade',
    }),
    episodeId: varchar('episode_id', { length: 255 }).references(() => tmdbEpisode.id, {
        onDelete: 'cascade',
    }),
    // --- THIS IS THE CHANGE ---
...
export const accounts = pgTable('account', {
    userId: varchar({ length: 255 })
        .notNull()
        .references(() => users.id),
    type: varchar({ length: 255 }).$type().notNull(),
...
export const userMediaList = pgTable('user_media_list', {
