import { tmdbEpisode, tmdbMedia, tmdbSeason, tmdbSource, tmdbTrending, userSubmission, userSubmissionStatusEnum, } from '~/server/db/schema';
import { and, asc, desc, eq, gt, inArray, isNull, lt, or, sql, } from 'drizzle-orm';
import { format, subDays } from 'date-fns';
import { createGunzip } from 'node:zlib';
import { Readable } from 'stream';
import readline from 'readline';
import { bulkUpdatePopularity, updateRatingsForMediaList, populateMediaUsingTmdbIds, bulkUpsertNewMedia, populateOriginForMediaList, populateSeasonAndEpisodeForTvList, } from '~/server/utils/mediaUtils';
import { db } from '../db';
import { fetchAllChangedTmdbIds, fetchTmdbListViaApi, findExistingTmdbIdsFromFetched, findNewMediaFromFetched, } from './tmdbApiUtils';
...
export async function updateAllChangedMedia() {
    ...
    console.log(`[updateMediaUsingTmdbChangedApi] number of changed media in tmdb: ${allChangedIds.length}`);
    ...
    console.log(`[updateMediaUsingTmdbChangedApi] number of changed media in db: ${changedIdsInMyDb.length}`);
...
async function updatePopularity(mediaType) {
    ...
    if (!tmdbResponse.ok || !tmdbResponse.body) {
        console.error(`[updatePopularity] Failed to download file: ${tmdbResponse.statusText}`);
        return;
    }
    ...
    Readable.fromWeb(tmdbResponse.body).pipe(gunzip);
...
export async function updateRatings() {
    ...
    const mediaToUpdate = await db
        ...
        .from(tmdbMedia)
        .where(or(isNull(tmdbMedia.voteUpdatedAt), // 1. Never updated
    and(
    // 2. Popular but stale
    gt(tmdbMedia.popularity, 20), lt(tmdbMedia.voteUpdatedAt, sql `CURRENT_DATE - INTERVAL '7 day'`)), lt(tmdbMedia.voteUpdatedAt, sql `CURRENT_DATE - INTERVAL '30 day'`) // 3. Very stale
    ))
    ...
    await db
        ...
        .where(inArray(tmdbMedia.id, mediaToUpdate.map((m) => m.id)));
    console.log(`[updateRatings] done: ${mediaToUpdate.length} required => ${successCount} successful.`);
}
export async function processUserSubmissions() {
    ...
    console.log(`[upsertUserSubmittedIds] Populated ${populateOutput.length} media from user submissions.`);
    ...
    const bulkUpdateSubmissions = async (batch) => {
        if (batch.length === 0)
            return;
        ...
        SET 
          ...
          processed_at = NOW()
        FROM (VALUES ${sql.join(batch.map((s) => sql `(${s.id}, ${s.status})`), sql `, `)}) AS data(id, new_status)
        WHERE ${userSubmission.id} = data.id::varchar;
      `);
    };
    // Call the helper to run the batch processing
    await runItemsInEachBatchInBulk(submissionResults, 10000, bulkUpdateSubmissions);
    const successCount = submissionResults.filter((s) => s.status === 'success').length;
    const failureCount = submissionResults.length - successCount;
    console.log(`[upsertUserSubmittedIds] db submission status updated: success (${successCount}) | failure (${failureCount}).`);
}
export async function populateMediaUsingTmdbLists(limit) {
...
export async function populateMediaUsingTmdbList(listType, limit) {
    ...
    console.log(`[populateMediaUsingTmdbList] fetched ${listType}: ${mvFetchOutput.length} mv | ${tvFetchOutput.length} tv`);
...
export async function fetchSrc() {
    ...
    const mediaToFetchSrc = await db
        ...
        .from(tmdbMedia)
        .where(or(isNull(tmdbMedia.srcFetchedAt), // 1. Never checked
    and(
    // 2. Popular but not checked recently (e.g., 3 days for sources)
    gt(tmdbMedia.popularity, 20), lt(tmdbMedia.srcFetchedAt, sql `CURRENT_DATE - INTERVAL '1 day'`)), 
...
export async function updateDenormFieldsForMediaList(input) {
    if (input !== 'all' && input.length === 0) {
        console.log('[updateDenormFieldsForMediaList] empty input.');
        return { updatedMovies: 0, updatedTvShows: 0 };
    }
    console.log(`[updateDenormFieldsForMediaList] input length: ${input === 'all' ? 'all' : input.length}`);
    // 1. Update mv
    const mvConditions = input === 'all'
    ...
    RETURNING m.id;
  `);
    console.log(`[updateDenormFieldsForMediaList] Updated ${mvOutput.length} movies`);
    // 2. get tv
    const tvConditions = [
    ...
    RETURNING m.id;
  `);
    console.log(`[updateDenormFieldsForMediaList] Updated ${tvOutput.length} tv.`);
    return {
