...
import { db } from '../db';
import { tmdbEpisode, tmdbMedia, tmdbMediaToTmdbGenre, tmdbMediaToTmdbOrigin, tmdbSeason, tmdbSource, tmdbSubtitle, tmdbTrending, } from '../db/schema';
import { and, asc, eq, inArray, isNotNull, isNull, lte, or, sql, } from 'drizzle-orm';
import { indexProviderMap, mvProvidersMap, tvProvidersMap, } from '~/server/utils/puppeteerUtils';
import { fetchTmdbDetailViaApi, fetchTmdbSeasonDetailViaApi, } from './tmdbApiUtils';
import { runItemsInEachBatchConcurrently } from './utils';
export async function bulkUpsertNewMedia(fetchOutput) {
    // 2. Prepare db input from api fetch ouput (removes duplicates)
    const mediaInput = Array.from(new Map(fetchOutput
    ...
    const tmdbIdToMediaIdMap = new Map(mediaOutput.map((item) => [item.tmdbId, item.mediaId]));
    ...
    const genreInput = mediaInput.flatMap((media) => {
        ...
        return media.genreIds.map((genreId) => ({
            mediaId: mediaId,
            genreId: genreId,
        }));
    });
    ...
    const originInput = mediaInput.flatMap((media) => {
        ...
        return media.originIds.map((originId) => ({
            mediaId: mediaId,
    ...
    console.log(`[bulkUpsertNewMedia] done upserting to media/origin/genre: ${mediaOutput.length}`);
    return mediaOutput;
}
// Helper function to be placed outside the router
export async function bulkUpdatePopularity(batch) {
    if (batch.length === 0)
        return;
    try {
      ...
      FROM (VALUES ${sql.join(batch.map((p) => sql `(${p.tmdbId}, ${p.popularity})`), sql `, `)}) AS data(tmdb_id, popularity)
...
export async function upsertSeasonsAndEpisodes(detail, media) {
    // console.log(
    //   `[upsertSeasonsAndEpisodes] tv ${media.tmdbId} (${media.title}) detail:`,
    //   detail
    // );
    const seasonsFromApi = detail.seasons?.filter((s) => s.season_number !== 0);
    ...
    if (media.seasonCount && media.seasonCount === seasonsFromApi.length) {
        console.log(`[upsertSeasonsAndEpisodes] tv ${media.tmdbId} (${media.title}): db ${media.seasonCount} = api ${seasonsFromApi.length}`);
        return;
    }
    const seasonDetailsPromises = seasonsFromApi.map((season) => fetchTmdbSeasonDetailViaApi(detail.id, season.season_number));
    const seasonDetails = await Promise.all(seasonDetailsPromises);
    // 2. Prepare all seasons for a single batch upsert
    const seasonInput = seasonsFromApi.map((season) => ({
    ...
    await db.transaction(async (tx) => {
        ...
        console.log(`[upsertSeasonsAndEpisodes] Upserting ${seasonInput.length} seasons...`);
        ...
        const seasonNumberToIdMap = new Map(seasonOutput.map((s) => [s.seasonNumber, s.id]));
        ...
        const episodeInput = seasonDetails.flatMap((seasonDetail) => {
            ...
            return seasonDetail.episodes.map((episode, index) => ({
                seasonId: seasonId,
        ...
        console.log(`[upsertSeasonsAndEpisodes] Upserting ${episodeInput.length} episodes...`);
        await tx
            .insert(tmdbEpisode)
            .values(episodeInput)
            .onConflictDoUpdate({
...
export async function populateSeasonAndEpisodeForTvList(tvInput, tvDetails) {
    console.log(`[populateSeasonAndEpisodeForTvList] input length: ${tvInput.length}`);
    console.log(`[populateSeasonAndEpisodeForTvList] details length: ${tvDetails.length}`);
    if (tvInput.length === 0)
        return;
    if (tvDetails.length === 0) {
        ...
        await runItemsInEachBatchConcurrently(tvInput, 10, async (tv) => {
            detailCount++;
            console.log(`[populateSeasonAndEpisodeForTvList] fetch tv detail: ${detailCount}/${tvInput.length} (${tv.tmdbId}:${tv.title})`);
            try {
                const detail = await fetchTmdbDetailViaApi(tv.type, tv.tmdbId);
                tvDetails.push(detail);
            }
            catch (err) {
                console.error(`[populateSeasonAndEpisodeForTvList] fetch tv detail failed: ${tv.tmdbId}`, err);
            }
        });
        console.log(`[populateSeasonAndEpisodeForTvList] fetched tv details: ${tvDetails.length}`);
    }
    let count = 0;
    await runItemsInEachBatchConcurrently(tvInput, 10, async (tv) => {
        count++;
        console.log(`[populateSeasonAndEpisodeForTvList] upsert tv seasons/episodes: ${count}/${tvInput.length} (${tv.tmdbId}:${tv.title})`);
        // get details
        const tvDetail = tvDetails.find((detail) => detail.id === tv.tmdbId);
        if (!tvDetail) {
            console.log(`[populateSeasonAndEpisodeForTvList] tv ${tv.title}: no details`);
            return;
        }
        // upsert seasons/episodes
        await upsertSeasonsAndEpisodes(tvDetail, tv);
    });
    console.log(`[populateSeasonAndEpisodeForTvList] upsert tv seasons/episodes: done`);
}
export async function populateOriginForMediaList(mediaList) {
    let count = 0;
    const originInput = [];
    await runItemsInEachBatchConcurrently(mediaList, 10, async (media) => {
        count++;
        console.log(`[populateOriginForMediaList] progress: ${count}/${mediaList.length} (${media.type} ${media.tmdbId}:${media.title})`);
        // 2. fetch detail via api
        const details = await fetchTmdbDetailViaApi(media.type, media.tmdbId);
        // 3. collect origin inputs
        const originInput = details.origin_country.map((originId) => ({
            mediaId: media.mediaId,
            originId: originId,
        }));
...
export async function populateMediaUsingTmdbIds(input) {
    // 1. batch fetch details from tmdb api and store details in one array
    console.log(`[populateMediaUsingTmdbIds] fetching media details: ${input.length}`);
    const mediaDetails = [];
    await runItemsInEachBatchConcurrently(input, 10, async (item) => {
        try {
            ...
            console.error(`[populateMediaUsingTmdbIds] failed fetching detail: ${item.tmdbId}`, err);
        }
    });
    console.log(`[populateMediaUsingTmdbIds] done fetching media details: ${mediaDetails.length}`);
    // 2. upsert media, genre, origin
    const mediaOutput = await bulkUpsertNewMedia(mediaDetails);
    console.log(`[populateMediaUsingTmdbIds] Upserted media/genre/origin: ${mediaOutput.length}`);
...
export async function updateRatingsForMediaList(input) {
    ...
    await runItemsInEachBatchConcurrently(input, 10, async (media) => {
        ...
        if (details?.vote_average && details?.vote_count) {
            ...
            console.log(`[updateRatingsForMediaList] ${media.type} (${media.title}): Missing ${details?.vote_average
                ? `vote vount`
                : details?.vote_count
                    ? `vote avg`
                    : `both count and avg`}`);
    ...
    console.log(`[updateRatingsForMediaList] API returned ${newRatings.length} ratings for media. Starting bulk db insert...`);
    ...
    try {
      ...
      UPDATE ${tmdbMedia} SET
        ...
        vote_updated_at = NOW()
      FROM (VALUES ${sql.join(newRatings.map((r) => sql `(${r.id}, ${r.voteAverage}, ${r.voteCount})`), sql `, `)}) AS data(id, vote_average, vote_count)
    ...
    console.log(`[updateRatingsForMediaList] Done: db updated ${newRatings.length} media ratings.`);
