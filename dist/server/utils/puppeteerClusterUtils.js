...
import { findAndClick, clickPlayInFrame, playButtonSelectorsMap, videoSelectorsMap, settingsButtonSelectorsMap, highestResolutionSelectorsMap, subtitleButtonSelectorsMap, subtitleTabSelectorsMap, enSubtitleSelectorsMap, firstClickMap, timeoutPromise, getTime, providerIndexMap, } from './puppeteerUtils'; // Assuming your helper functions and maps are in a utils file
/**
 * This is the main task logic, extracted from your old API endpoint.
 * It's defined once and used by the cluster for all jobs.
 */
const puppeteerTask = async ({ page, data }) => {
    ...
    page.on('response', async (res) => {
        ...
        const isSubtitle = url.includes('format=srt&encoding=UTF-8') || url.includes('.vtt');
    ...
    try {
        ...
        try {
            ...
            if (provider === 'vidjoy') {
                ...
                await click('highest resolution', highestResolutionSelectorsMap[provider]);
            }
            ...
            console.warn(`[${provider}] ${error.message} ${error.message.includes(firstClickMap[provider]) ? `(1st click)` : ``}`);
        }
        await timeoutPromise(provider === 'vidjoy'
