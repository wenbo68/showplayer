...
import { tmdbEpisode, tmdbMedia, tmdbSeason, tmdbSource, tmdbSubtitle, } from '../db/schema';
import { eq, and, asc, inArray, isNotNull, isNull, lte, sql, } from 'drizzle-orm';
import { db } from '../db';
import { runItemsInEachBatchConcurrently } from './utils';
// import { runItemsInEachBatchConcurrently } from './mediaUtils';
function convertToVtt(subtitle) {
 ...
 */
async function fetchSrcFromProvidersFast(type, path) {
    try {
        ...
        const promises = providers.map((provider) => {
            const embedUrl = `${type === 'mv' ? mvProvidersMap[provider] : tvProvidersMap[provider]}/${path}`;
        ...
        successfulResults.forEach((result) => {
            console.log(`[${indexProviderMap[result.provider]}] success: ${result.m3u8.type} ${result.subtitle ? '+ subtitle' : ''}`);
        });
        ...
        console.error(`[fetchSrcFromProvidersFast] A critical error occurred:`, error);
        return []; // Return empty array on failure
    }
}
async function upsertSrcAndSubtitle(type, id, results) {
    ...
    if (sourceIds.length === 0)
        return; // Nothing to do for subtitles
...
tmdbId) {
    ...
    console.log('=======');
    console.log(`[fetchAndUpsertMvSrc] Fetched ${results.length} sources: ${results.map((result) => indexProviderMap[result.provider])}`);
    if (results.length === 0) {
        return;
    }
...
// spd: 'fast' | 'slow',
mediaId, tmdbId, season, episode, episodeIndex) {
    // let results =
    //   spd === 'fast'
    //     ? await fetchSrcFromProvidersFast('tv', `${tmdbId}/${season}/${episode}`)
    //     : await fetchSrcFromProvidersSlow('tv', `${tmdbId}/${season}/${episode}`);
    let results = await fetchSrcFromProvidersFast('tv', `${tmdbId}/${season}/${episode}`);
    console.log('=======');
    console.log(`[fetchAndUpsertTvSrc] Fetched ${results.length} sources: ${results.map((result) => indexProviderMap[result.provider])}`);
    if (results.length === 0) {
        if (episode === episodeIndex)
            return;
        console.log(`[fetchAndUpsertMvSrc] Trying episode index: ${tmdbId}/${season}/${episodeIndex}`);
        // results = spd
        //   ? await fetchSrcFromProvidersFast('tv', `${tmdbId}/${season}/${episode}`)
        //   : await fetchSrcFromProvidersSlow('tv', `${tmdbId}/${season}/${episode}`);
        results = await fetchSrcFromProvidersFast('tv', `${tmdbId}/${season}/${episodeIndex}`);
        if (results.length === 0)
            return;
    }
    // insert sources into tmdbSource table
    ...
    const [episodeData] = await db
        ...
        .where(and(eq(tmdbMedia.tmdbId, tmdbId), eq(tmdbSeason.seasonNumber, season), eq(tmdbEpisode.episodeNumber, episode)))
        .limit(1);
    // 2. If the episode doesn't exist in your DB, throw an error.
    if (!episodeData) {
        throw new Error(`[fetchAndUpsertTcSrc] failed: episode not found for ${tmdbId}/${season}/${episode}`);
    }
    // 3. upsert sources and subtitles
...
export async function fetchSrcForMediaIds(input) {
    if (input.length === 0) {
    ...
    const airedEpisodeSubquery = db
        ...
        episodeCount: sql `count(${tmdbEpisode.id})`.as('episode_count'),
    })
        .from(tmdbEpisode)
        .innerJoin(tmdbSeason, eq(tmdbEpisode.seasonId, tmdbSeason.id))
        .where(and(isNotNull(tmdbEpisode.airDate), lte(tmdbEpisode.airDate, yesterday)))
    ...
    const srclessEpisodes = await db
        ...
        .innerJoin(tmdbSeason, eq(tmdbEpisode.seasonId, tmdbSeason.id))
        .innerJoin(tmdbMedia, eq(tmdbSeason.mediaId, tmdbMedia.id))
        .innerJoin(airedEpisodeSubquery, eq(tmdbMedia.id, airedEpisodeSubquery.mediaId))
        .leftJoin(tmdbSource, eq(tmdbSource.episodeId, tmdbEpisode.id))
        .where(and(...tvConditions))
        .orderBy(asc(airedEpisodeSubquery.episodeCount), asc(tmdbMedia.tmdbId), asc(tmdbSeason.seasonNumber), asc(tmdbEpisode.episodeNumber));
    ...
    try {
        ...
        await runItemsInEachBatchConcurrently(srclessMv, 1, async (media) => {
            processedMvCount++;
            console.log(`=======`);
            console.log(`[fetchSrcForMediaList] mv progress: ${processedMvCount}/${srclessMv.length} (${media.tmdbId}: ${media.title})`);
        ...
        await runItemsInEachBatchConcurrently(srclessEpisodes, 1, async (item) => {
            ...
            console.log(`=======`);
            console.log(`[fetchSrcForMediaList] tv progress: ${processedEpisodeCount}/${srclessEpisodes.length} (${item.media.tmdbId}/${item.season.seasonNumber}/${item.episode.episodeNumber}: ${item.media.title}) (${item.airedEpisodeCount})`);
            const { episode, season, media } = item;
            await fetchAndUpsertTvSrc(media.id, media.tmdbId, season.seasonNumber, episode.episodeNumber, episode.episodeIndex);
        });
    }
    finally {
