 ...
 */
async function fetchChangedTmdbIds(mediaType) {
    ...
    for (let i = 0; i < pagesToFetch.length; i += batchSize) {
        ...
        const batchFetchPromises = pageBatch.map((page) => fetch(`${baseUrl}?page=${page}`, { headers: TMDB_API_HEADERS }));
        ...
        for (const res of responses) {
            if (res.ok) {
                ...
                console.warn(`[fetchChangedIdsForType] Failed to fetch a page for ${mediaType}. Status: ${res.status}`);
            }
        }
        console.log(`[fetchChangedIdsForType] ${mediaType} progress: ${i + 1}/${totalPages} pages`);
    }
 ...
 */
export async function fetchAllChangedTmdbIds() {
    try {
 ...
 */
export async function findExistingTmdbIdsFromFetched(input) {
    if (input.length === 0) {
        return [];
    }
    ...
    const compositeWhereClause = or(...input.map((id) => and(eq(tmdbMedia.tmdbId, id.tmdbId), eq(tmdbMedia.type, id.type))));
    ...
    const existingMedia = await db
        ...
        seasonCount: sql `count(${tmdbSeason.id})`.mapWith(Number),
    })
        .from(tmdbMedia)
 ...
 */
export async function findNewMediaFromFetched(fetchOutput) {
    ...
    console.log(`[findNewMedia] ${fetchOutput.length} fetched => ${newMedia.length} new`);
    return newMedia;
}
export async function fetchTmdbTopRatedViaApi(limit, type) {
    const collected = [];
    const seenIds = new Set(); // Use a Set to track seen IDs
    let page = 1;
    while (collected.length < limit) {
        const resp = await fetch(`https://api.themoviedb.org/3/${type}/top_rated?language=en-US&page=${page}`, {
            headers: {
                accept: 'application/json',
...
export async function fetchTmdbListViaApi(listType, mediaType, limit) {
    const collected = [];
    const seenIds = new Set(); // Use a Set to track seen IDs
    let page = 1;
    while (collected.length < limit) {
        const resp = await fetch(`https://api.themoviedb.org/3/${listType === 'trending'
...
export async function fetchTmdbDetailViaApi(type, id) {
    const resp = await fetch(`https://api.themoviedb.org/3/${type}/${id}?language=en-US`, {
        headers: {
            accept: 'application/json',
            Authorization: `Bearer ${process.env.TMDB_API_KEY}`,
        },
...
export async function fetchTmdbOriginsViaApi() {
    const resp = await fetch(`https://api.themoviedb.org/3/configuration/countries?language=en-US`, {
        headers: {
            accept: 'application/json',
            Authorization: `Bearer ${process.env.TMDB_API_KEY}`,
        },
...
export async function fetchTmdbMvGenresViaApi() {
    const resp = await fetch(`https://api.themoviedb.org/3/genre/movie/list?language=en`, {
        headers: {
            accept: 'application/json',
            Authorization: `Bearer ${process.env.TMDB_API_KEY}`,
        },
...
export async function fetchTmdbTvGenresViaApi() {
    const resp = await fetch(`https://api.themoviedb.org/3/genre/tv/list?language=en`, {
        headers: {
            accept: 'application/json',
            Authorization: `Bearer ${process.env.TMDB_API_KEY}`,
        },
...
export async function fetchTmdbSeasonDetailViaApi(id, seasonNumber) {
    const resp = await fetch(`https://api.themoviedb.org/3/tv/${id}/season/${seasonNumber}?language=en-US`, {
        headers: {
            accept: 'application/json',
            Authorization: `Bearer ${process.env.TMDB_API_KEY}`,
        },
