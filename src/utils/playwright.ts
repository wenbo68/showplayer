...
async function attemptClick(
  actionFn: (src: string, page: Page) => Promise<void>,
  label: string,
  maxAttempt: number,
  src: string,
  page: Page,
  getPopupCount: () => number // Pass a function to get the current count
) {
  console.log(`Finding ${label}...`);
  for (let attempt = 1; attempt <= maxAttempt; attempt++) {
    console.log(`-attempt #${attempt}`);

    const popupsBeforeClick = getPopupCount();
    console.log(`before: `, popupsBeforeClick);
    await actionFn(src, page);
    await new Promise((res) => setTimeout(res, 5000));
    const popupsAfterClick = getPopupCount();
    console.log(`after: `, popupsAfterClick);

    if (popupsAfterClick === popupsBeforeClick) {
...
async function fetchSrcFromUrl(
  ...
  try {
    ...
    const getPopupCount = () => popupsClosedCount;
    if (src === 'videasy' || src === 'vidsrc') {
      await attemptClick(clickPlay, 'play button', 5, src, page, getPopupCount);
      await attemptClick(
        clickSubtitleMenu,
        'subtitle menu',
        5,
